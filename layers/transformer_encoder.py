...
class PatchTransfromerEncoder(nn.Module):
    def __init__(self, c_in, num_patch, patch_len, 
                 n_layers=3, d_model=512, n_heads=16, shared_embedding=True,
                 d_ff=2048, norm='BatchNorm', attn_dropout=0., dropout=0., act="gelu", store_attn=False,
    ...
    def forward(self, x) -> Tensor:          
        """
        x: tensor [bs x nvars x num_patch x patch_len] | [bs x num_patch x patch_len]
        """
        # ===
        if x.ndim == 3:
            x = x.unsqueeze(dim=1)
...
if __name__ == "__main__":
    encoderlayer = PatchTransfromerEncoder(c_in=1, num_patch=20, patch_len=5, n_layers=3, d_model=512, n_heads=8, d_ff=2048)
    input = torch.rand(4, 20, 5)
    out = encoderlayer(input)
    print(out.shape)
