...
import numpy as np
from ._transforms import CompressTransform, StretchTransform, AnomalyTransform
...
if __name__ == "__main__":
    ...
    data = gt.sine(length=10000, frequency=1, amplitude=1)
    data += np.random.normal(scale=0.05, size=len(data))
    # generate injections
    rng = np.random.default_rng(42)
    np.random.seed(42)
    data_abnorm, labels = inject_anomalies(data, rng=rng)

    plt.figure()
    # plt.plot(data, label="original")
    # plt.plot(data_abnorm, label="anomaly injected")
