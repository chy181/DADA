...
class BottleNeck(nn.Module):
    ...
    def __init__(self, seq_len, repr_dim, bn_dim):
        ...
        # lora
        # self.net = nn.Sequential(
        #     lora.Linear(repr_dim, bn_dim, r=16),
        #     nn.GELU(),
        #     lora.Linear(bn_dim, bn_dim, r=16),
        #     nn.GELU(),
        #     nn.Dropout(0.1),
        #     lora.Linear(bn_dim, repr_dim, r=16),
        # )
...
if __name__ == "__main__":
    x = torch.rand(4*9, 20*5) # b*c x pN*pL
    repr = torch.rand(4*9, 20*256) # b*c x pN*repr_dim
    # usage
    model = AdaptiveBottleNeck(seq_len=20, seq_dim=256, repr_dim=256, bn_dims=[8, 16, 32], k=2)
    repr, loss = model(repr, repr)
    # print(repr.shape)
